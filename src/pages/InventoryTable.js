...
function InventoryTable() {
  ...
  const fetchItems = async () => {
    ...
    if (!(user.role === 'admin' && selectedUnit === 'Administration')) {
      ...
      query = query.eq('unit', unitToFetch);
    }
 ...
 const uploadToSupabase = async (rows) => {
  const formatted = rows.map(row => {
    ...
    return {
      ...
      date_received: row['DATE RECEIVED'] || row.date_received || '',
  ...
  return (
    <div className="p-6">
      <h2 className="text-xl font-bold mb-4">
        Inventory Overview — {user.role === 'admin' ? (selectedUnit === 'Administration' ? 'All Units' : selectedUnit) : user.unit}
      </h2>
